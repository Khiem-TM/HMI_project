...
import type { FaceStateModel, SWFeatureDescription } from '@/lib/stores/signwriting-store';
...
class FaceService {
  private faceSequentialModel: LayersModel | null = null;

  async loadModel(): Promise<void> {
    await Promise.all([tfjsService.load(), threeService.load()]);

    try {
      this.faceSequentialModel = (await tfjsService.loadLayersModel(
        '/assets/models/face-features/model.json'
      )) as LayersModel;
    } catch (e) {
      console.warn('Face features model not available:', e);
    }
  }

  async normalize(vectors: Vector3[]): Promise<Tensor> {
    const normal = await poseNormalizationService.normal(vectors, [4, 133, 362]);
    return poseNormalizationService.normalize(vectors, normal, [4, 6], 4);
  }

  async shape(vectors: Vector3[]): Promise<FaceStateModel> {
    await threeService.load();
    const { Vector2 } = threeService;
  ...
  private shift(char: string, shift: number): string {
    return String.fromCodePoint((char.codePointAt(0) || 0) + shift);
  }
}
