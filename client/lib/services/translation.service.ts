...
class TranslationService {
  ...
  async createModelRegistry(modelPath: string, modelName: string = 'enase'): Promise<ModelRegistry> {
    ...
    if (modelName === 'spokensigned') {
      // Generic model
      fileMap = {
        model: { name: `/assets/${modelPath}model.spokensigned.intgemm.alphas.bin` },
      ...
      fileMap = {
        model: { name: `/assets/${modelPath}model.${modelName}.intgemm.alphas.bin` },
  ...
  async loadOfflineModel(
    ...
    try {
      ...
      throw new Error(`Failed to load ${useGeneric ? 'generic' : 'specific'} model: ${error.message}`);
    }
  }
  ...
  async translateOffline(
    ...
    let translations = await this.worker.translate(translateFrom, translateTo, [text], [{ isHtml: false }]);
    ...
    } else if (translationTexts[0] && typeof translationTexts[0] === 'object' && 'text' in translationTexts[0]) {
      translationText = (translationTexts[0] as any).text;
    } else {
      throw new Error(`Unexpected translation result format: ${typeof translationTexts[0]}`);
    }
  ...
  async translateSpokenToSignWriting(
    ...
    try {
      ...
      return await this.translateOffline(direction, formattedText, spokenLanguage, signedLanguage, true);
    } catch (error) {
  ...
  getSpokenToSignedVideoUrl(
    ...
    const api = 'https://us-central1-sign-mt.cloudfunctions.net/spoken_text_to_signed_video';
    return `${api}?text=${encodeURIComponent(text)}&spoken=${spokenLanguage}&signed=${signedLanguage}`;
  }
  ...
  getSignedToSpokenTextUrl(
    ...
    const api = 'https://us-central1-sign-mt.cloudfunctions.net/signed_video_to_spoken_text';
    return `${api}?video=${encodeURIComponent(videoUrl)}&signed=${signedLanguage}&spoken=${spokenLanguage}`;
  }
  ...
  async translateSignedToSpokenText(
    ...
    try {
      const apiUrl = this.getSignedToSpokenTextUrl(videoUrl, signedLanguage, spokenLanguage);
  ...
  async translateSignedLandmarksToText(
    ...
    // For now, this would need to use the offline model with direction 'signed-to-spoken'
    // But sign-to-text typically requires video/pose data, not just landmarks
    // This is a placeholder for future implementation
    throw new Error('Landmark-based sign-to-text translation not yet implemented. Please use video URL instead.');
  }
}
